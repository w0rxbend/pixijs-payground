...
export class ParticleSplashScreen extends Container {
  ...
  private spawnOrbs(): void {
    for (let i = 0; i < ORB_COUNT; i++) {
      ...
      this.orbs.push({
        ...
        radius: ORB_BASE_RADIUS + Math.random() * (ORB_MAX_RADIUS - ORB_BASE_RADIUS),
  ...
  private tickOrbs(dt: number): void {
    for (const orb of this.orbs) {
      // Wander steering
      orb.wanderAngle += (Math.random() - 0.5) * 2 * WANDER_JITTER * dt * 60 * dt;
      ...
      const wanderFx = (circleCx + Math.cos(orb.wanderAngle) * WANDER_RADIUS - orb.x) * 0.12;
      const wanderFy = (circleCy + Math.sin(orb.wanderAngle) * WANDER_RADIUS - orb.y) * 0.12;
      ...
      let bfx = 0, bfy = 0;
      if (orb.x < BOUNDARY_MARGIN)        bfx += BOUNDARY_FORCE * (1 - orb.x / BOUNDARY_MARGIN);
      if (orb.x > this.w - BOUNDARY_MARGIN) bfx -= BOUNDARY_FORCE * (1 - (this.w - orb.x) / BOUNDARY_MARGIN);
      if (orb.y < BOUNDARY_MARGIN)        bfy += BOUNDARY_FORCE * (1 - orb.y / BOUNDARY_MARGIN);
      if (orb.y > this.h - BOUNDARY_MARGIN) bfy -= BOUNDARY_FORCE * (1 - (this.h - orb.y) / BOUNDARY_MARGIN);
      ...
      if (spd > MAX_SPEED) { orb.vx = (orb.vx / spd) * MAX_SPEED; orb.vy = (orb.vy / spd) * MAX_SPEED; }
      if (spd < MIN_SPEED && spd > 0.01) { orb.vx = (orb.vx / spd) * MIN_SPEED; orb.vy = (orb.vy / spd) * MIN_SPEED; }
  ...
  private drawSparks(g: Graphics): void {
    for (const s of this.sparks) {
      ...
      for (let i = 1; i < s.trail.length; i++) {
        ...
        g.moveTo(prev.x, prev.y).lineTo(curr.x, curr.y)
          .stroke({ width: s.radius * tt, color: s.color, alpha: tt * 0.7 });
      }

      // Core spark
      g.circle(s.x, s.y, s.radius * (0.5 + t * 0.5)).fill({ color: s.color, alpha: alpha * 0.9 });
      g.circle(s.x, s.y, s.radius * 2.5 * t).fill({ color: s.color, alpha: alpha * 0.15 });
    }
  }

  private drawOrbs(g: Graphics): void {
    for (const orb of this.orbs) {
      const pulse = 0.85 + 0.15 * Math.sin(this.time * orb.pulseSpeed + orb.pulsePhase);
      ...
      // Bright specular dot
      g.circle(orb.x - r * 0.28, orb.y - r * 0.28, r * 0.35).fill({ color: 0xffffff, alpha: 0.55 });

      // Outer rim for depth
      g.circle(orb.x, orb.y, r)
        .stroke({ width: 0.8, color: C_OVERLAY0, alpha: 0.15 });
    }
  }
}
