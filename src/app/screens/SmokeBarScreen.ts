...
function smokeColor(y: number): number {
  const t = y / H;
  if (t > 0.80) return 0x11111b;
  if (t > 0.62) return 0x181825;
  if (t > 0.46) return 0x1e1e2e;
  if (t > 0.30) return 0x313244;
...
const TEXT_STR     = " Need for Speed™ Heat >>> smoking tires >  burning rubber > drifting corners >>> racing hard! >>>   ";
...
interface Puff {
  ...
  r: number; r0: number;
  rx: number; ry: number;
  wobbleA: number; wobbleF: number; wobbleP: number;
  life: number; lifeSpd: number;
  sprite: Sprite;
}

const TIERS = [
  { yLo: H * 0.40, yHi: H * 1.20, rLo: 20, rHi: 36, vxLo: 0.55, vxHi: 1.30, burst: [5,  9] },
  { yLo: H * 0.20, yHi: H * 0.65, rLo:  8, rHi: 18, vxLo: 0.80, vxHi: 2.00, burst: [4,  7] },
  { yLo: H * 0.00, yHi: H * 0.35, rLo:  2, rHi:  8, vxLo: 1.40, vxHi: 3.50, burst: [5, 10] },
] as const;
...
function buildWheelGraphic(r: number): Graphics {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const rI = r - 5, rO = r + 2.5, hw = 3.6;
    g.poly([
      rI*cos+hw*sin, rI*sin-hw*cos,
      rI*cos-hw*sin, rI*sin+hw*cos,
      rO*cos-hw*sin, rO*sin+hw*cos,
      rO*cos+hw*sin, rO*sin-hw*cos,
    ]).fill({ color: 0x1e1e2e });
  }
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const cos = Math.cos(a), sin = Math.sin(a), hw = 1.4;
    g.poly([
      rd*0.42*cos+hw*sin, rd*0.42*sin-hw*cos,
      rd*0.42*cos-hw*sin, rd*0.42*sin+hw*cos,
      rd*0.90*cos-hw*sin, rd*0.90*sin+hw*cos,
      rd*0.90*cos+hw*sin, rd*0.90*sin-hw*cos,
    ]).fill({ color: 0x181826 });
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const cos = Math.cos(a), sin = Math.sin(a);
    const px = -sin, py = cos;
    const rN = hub * 1.75, rF = ri * 0.88;
    const wN = ri * 0.195, wF = ri * 0.075;
    // Shadow half
    g.poly([
      cos*rN+px*wN*0.05, sin*rN+py*wN*0.05,
      cos*rN-px*wN,      sin*rN-py*wN,
      cos*rF-px*wF,      sin*rF-py*wF,
      cos*rF+px*wF*0.05, sin*rF+py*wF*0.05,
    ]).fill({ color: 0x23253a });
    // Main body
    g.poly([
      cos*rN+px*wN,      sin*rN+py*wN,
    ...
    // Highlight
    const hn = wN*0.45, hf = wF*0.45;
    g.poly([
      cos*rN+px*hn,      sin*rN+py*hn,
      cos*rN+px*hn*0.1,  sin*rN+py*hn*0.1,
      cos*rF+px*hf*0.1,  sin*rF+py*hf*0.1,
      cos*rF+px*hf,      sin*rF+py*hf,
  ...
  g.circle(0, 0, ri * 0.90).stroke({ color: 0x44465e, width: 1 });
...
export class SmokeBarScreen extends Container {
    ...
    for (const tier of TIERS) {
      ...
      for (let i = 0; i < n && this.puffs.length < MAX_PUFFS; i++) {
        ...
        const p: Puff = {
          ...
          r: r0, r0,
          rx: rnd(0.80, 1.25), ry: rnd(0.80, 1.25),
          wobbleA: rnd(1.5, 4.5), wobbleF: rnd(0.018, 0.05), wobbleP: rnd(0, Math.PI * 2),
          life: initLife,
          lifeSpd: rnd(0.00035, 0.00090),
          sprite: this.acquire(),
        };
        if (initLife > 0) {
          const frames = initLife / p.lifeSpd;
          p.x += p.vx * frames;
          p.r  = Math.max(1.5, p.r0 * (1 - (p.x / ((this.screenW || W) * 0.9)) * 0.55));
        }
  ...
  private scrollStep(deltaMS: number): void {
    ...
    if (c.length >= 2) { c[0].x = -this.scrollX; c[1].x = this.textW - this.scrollX; }
  }
  ...
  private syncSprites(): void {
    ...
    for (const p of this.puffs) {
      ...
      const rightFade = p.x <= fade ? 1.0 : 1.0 - (p.x - fade) / (w - fade) * 0.78;
