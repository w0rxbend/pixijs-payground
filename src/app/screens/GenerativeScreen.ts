...
const DRAW_COLORS = [MAUVE, SAPPH, TEAL, PEACH, PINK, GREEN, SKY, LAVEN, YELLOW, CATT_RED, TOXIC_GREEN] as const;

function randColor(): number { return DRAW_COLORS[Math.floor(Math.random() * DRAW_COLORS.length)]; }
  ...
  private _spawnPos(): { x: number; y: number } {
    if (Math.random() < EDGE_BIAS) {
      ...
      switch (Math.floor(Math.random() * 4)) {
        case 0: return { x: Math.random() * this.w, y: Math.random() * m };
        case 1: return { x: Math.random() * this.w, y: this.h - Math.random() * m };
        case 2: return { x: Math.random() * m,      y: Math.random() * this.h };
        default: return { x: this.w - Math.random() * m, y: Math.random() * this.h };
      }
    }
  ...
  public update(ticker: Ticker): void {
    ...
    for (const d of this.dots) {
      ...
      for (let i = 1; i < n; i++) {
        ...
        g.stroke({ color: d.color, width: d.radius * (0.3 + t * 0.7), alpha: t * t });
      }
