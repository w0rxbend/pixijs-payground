...
interface Star {
  ...
  color: number; trail: [number, number][];
}
const STAR_N     = 60;
const STAR_TRAIL = 8;
const STAR_G     = 200;
...
interface FieldPt { x: number; y: number; trail: [number, number][]; color: number; }
const MF_N       = 120;
const MF_TRAIL   = 6;
const MF_DIPO_R  = 300;
...
interface Boid { x: number; y: number; vx: number; vy: number; trail: [number, number][]; color: number; }
const BD_N      = 80;
const BD_TRAIL  = 12;
const BD_MAX_V  = 85;
const BD_SEP_R  = 25;
...
interface WaveSrc { angle: number; orbitSpeed: number; phase: number; }
...
function diskPt(a: number, b: number, t: number): { x: number; y: number } {
  const rx = a * Math.cos(t), ry = b * Math.sin(t);
  return { x: rx * DCOS - ry * DSIN, y: rx * DSIN + ry * DCOS };
}

interface DiskRing { a: number; b: number; color: number; alpha: number; w: number; }
const DISK_RINGS: readonly DiskRing[] = [
  { a: WEBCAM_R * 1.04, b: WEBCAM_R * 0.26, color: YELLOW,   alpha: 0.85, w: 2.0 },
  { a: WEBCAM_R * 1.09, b: WEBCAM_R * 0.28, color: YELLOW,   alpha: 0.78, w: 3.0 },
  { a: WEBCAM_R * 1.15, b: WEBCAM_R * 0.29, color: PEACH,    alpha: 0.72, w: 2.5 },
  { a: WEBCAM_R * 1.22, b: WEBCAM_R * 0.31, color: PINK,     alpha: 0.63, w: 2.0 },
  { a: WEBCAM_R * 1.30, b: WEBCAM_R * 0.33, color: MAUVE,    alpha: 0.52, w: 1.8 },
  { a: WEBCAM_R * 1.40, b: WEBCAM_R * 0.36, color: LAVENDER, alpha: 0.40, w: 1.5 },
  { a: WEBCAM_R * 1.52, b: WEBCAM_R * 0.39, color: SURFACE0, alpha: 0.27, w: 1.2 },
...
export class CosmicPortalScreen extends Container {
  ...
  // Clifford attractor
  private caA = -1.4; private caB = 1.6; private caC = 1.0; private caD = 0.7;
  private caX = rand(-1, 1); private caY = rand(-1, 1);
  ...
  constructor() {
    ...
    for (const g of [
      this.rdGfx, this.attractGfx, this.starGfx, this.fieldGfx,
      this.boidGfx, this.waveGfx, this.diskBgGfx,
      this.holeGfx,                                // punch-through
      this.lensingGfx, this.diskFgGfx,             // above hole
  ...
  private _initField(): void {
    for (let i = 0; i < MF_N; i++) {
      const a = rand(0, TAU), r = rand(WEBCAM_R + 10, 370);
      this.field.push({
        x: Math.cos(a) * r, y: Math.sin(a) * r,
        trail: [], color: MF_COLS[i % MF_COLS.length],
      });
    }
  }
  ...
  private _initBoids(): void {
    for (let i = 0; i < BD_N; i++) {
      const a = rand(0, TAU), r = rand(WEBCAM_R + 20, 360);
      const va = rand(0, TAU);
      this.boids.push({
        x: Math.cos(a) * r,   y: Math.sin(a) * r,
        vx: Math.cos(va) * 40, vy: Math.sin(va) * 40,
        trail: [],             color: BD_COLS[i % BD_COLS.length],
      });
    }
  }
  ...
  private _updateRD(): void {
    const N = RD_N;
    const U = this.rdU, V = this.rdV, U2 = this.rdU2, V2 = this.rdV2;
    ...
    for (let j = 0; j < N; j++) {
      for (let i = 0; i < N; i++) {
        ...
        const jl = (j - 1 + N) % N, jr = (j + 1) % N;
        const lapU = U[j*N+il] + U[j*N+ir] + U[jl*N+i] + U[jr*N+i] - 4*u;
        const lapV = V[j*N+il] + V[j*N+ir] + V[jl*N+i] + V[jr*N+i] - 4*v;
        U2[idx] = Math.max(0, Math.min(1, u + (RD_DU * lapU - uvv + RD_F * (1 - u)) * 0.5));
        V2[idx] = Math.max(0, Math.min(1, v + (RD_DV * lapV + uvv - (RD_F + RD_K) * v) * 0.5));
      }
    }
    this.rdU = U2; this.rdV = V2; this.rdU2 = U; this.rdV2 = V;

    const g = this.rdGfx;
    g.clear();
    const cw = this.w / N, ch = this.h / N;
  ...
  private _updateStars(dt: number): void {
    ...
    for (const s of stars) {
      ...
      // Star core
      g.circle(s.x, s.y, 1.5 + s.mass * 0.4).fill({ color: s.color, alpha: 0.95 });
      g.circle(s.x, s.y, 3.0 + s.mass * 0.7).fill({ color: s.color, alpha: 0.18 });
    }
  }
  ...
  private _dipoleField(x: number, y: number): [number, number] {
    let bx = 0, by = 0;
    for (let d = 0; d < 2; d++) {
      const a  = this.time * 0.30 + d * Math.PI;
  ...
  private _updateField(dt: number): void {
    ...
    for (const p of this.field) {
      ...
      for (let t = 0; t < p.trail.length; t++) {
        ...
        g.circle(tx, ty, 1.0).fill({ color: p.color, alpha: (t / p.trail.length) * 0.50 });
      }
    }
  }
  ...
  private _updateBoids(dt: number): void {
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let j = 0; j < N; j++) {
        ...
        if (d < BD_SEP_R) { sx -= dx / d; sy -= dy / d; ns++; }
        if (d < BD_ALI_R) { ax += o.vx;   ay += o.vy;   na++; }
        if (d < BD_COH_R) { cx += o.x;    cy += o.y;    nc++; }
      }

      let fax = 0, fay = 0;
      if (ns > 0) { fax += (sx / ns) * 2.2; fay += (sy / ns) * 2.2; }
      if (na > 0) { fax += (ax / na - b.vx) * 0.45; fay += (ay / na - b.vy) * 0.45; }
      if (nc > 0) { fax += (cx / nc - b.x)  * 0.012; fay += (cy / nc - b.y)  * 0.012; }
      ...
      if (r2 < repR * repR) {
        ...
        fax -= (b.x / r) * 180; fay -= (b.y / r) * 180;
      }
      // Keep on canvas
      if (r2 > 380 * 380) {
        const r = Math.sqrt(r2) + 0.001;
        fax -= (b.x / r) * 60; fay -= (b.y / r) * 60;
      }

      b.vx += fax * dt; b.vy += fay * dt;
      const spd = Math.sqrt(b.vx * b.vx + b.vy * b.vy) + 0.001;
      if (spd > BD_MAX_V) { b.vx = (b.vx / spd) * BD_MAX_V; b.vy = (b.vy / spd) * BD_MAX_V; }
      if (spd < 25)       { b.vx *= 1.06; b.vy *= 1.06; }
    ...
    for (const b of boids) {
      for (let t = 0; t < b.trail.length; t++) {
        ...
        g.circle(tx, ty, 1.5).fill({ color: b.color, alpha: (t / b.trail.length) * 0.60 });
      }
  ...
  private _updateWaves(): void {
    for (const src of this.waveSrcs) {
      src.angle += src.orbitSpeed * 0.010;
      src.phase += 0.045;
    }
    ...
    for (let ri = 0; ri < RINGS; ri++) {
      ...
      for (let si = 0; si < SEGS; si++) {
        ...
        for (const src of this.waveSrcs) {
          ...
          amp += Math.sin(d * 0.080 - this.time * 2.8 + src.phase);
        }
        amp /= 3;
  ...
  private _diskBuildPts(
    ring: DiskRing, tStart: number, tEnd: number, skipCam: boolean,
  ...
  private _diskDrawArc(
    g: Graphics, ring: DiskRing,
    tStart: number, tEnd: number, alphaScale: number, skipCam: boolean,
  ): void {
  ...
  private _updateDisk(): void {
    ...
    for (let i = 0; i < LAYS; i++) {
      ...
      const t  = i / (LAYS - 1);
      bg.circle(0, 0, r);
      bg.stroke({ color: CRUST, width: lh * 2.4, alpha: Math.pow(1 - t, 1.6) * 0.58 });
    }
    ...
    ringPath(lr); lg.stroke({ color: BLUE,     width: 18, alpha: sh1 * 0.07 });
    ringPath(lr); lg.stroke({ color: LAVENDER, width: 7,  alpha: sh1 * 0.22 });
    ringPath(lr); lg.stroke({ color: BLUE,     width: 2,  alpha: sh2 * 0.82 });
